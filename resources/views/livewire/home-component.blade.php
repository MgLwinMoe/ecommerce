                    ...
                    @foreach ($sproducts as $sproduct)
                        <div class="product product-style-2 equal-elem ">
                            ...
                            <div class="product-info">
                                ...
                                <div class="wrap-price"><ins><span class="sale_price">${{$sproduct->sale_price}}</span></ins><del><span class="product-price">${{$sproduct->regular_price}}</span></del></div>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="wrap-product-tab tab-style-1">
                    <div class="tab-control">
                        @foreach ($categories as $key=>$category )
                            <a href="#cateory_{{$category->id}}" class="tab-control-item {{$key == 0 ? 'active' : ''}}">{{$category->name}}</a>
                        @endforeach
                    </div>

                    <div class="tab-contents">
                        @foreach ($categories as $key=>$category )
                        <div class="tab-content-item {{$key == 0 ? 'active' : ''}}" id="cateory_{{$category->id}}">
                            <div class="wrap-products slide-carousel owl-carousel style-nav-1 equal-container" data-items="5" data-loop="false" data-nav="true" data-dots="false" data-responsive='{"0":{"items":"1"},"480":{"items":"2"},"768":{"items":"3"},"992":{"items":"4"},"1200":{"items":"5"}}' >
                                @php
                                    $c_products = DB::table('products')->where('category_id', $category->id)->get()->take($no_of_products);
                                @endphp

                                @foreach ($c_products as $cproducts)
                                <div class="product product-style-2 equal-elem ">
                                    <div class="product-thumnail">
                                        <a href="{{route('product.details',['slug' => $cproducts->slug])}}" title="{{$cproducts->name}}">
                                            <figure><img src="{{asset('assets/images/products')}}/{{$cproducts->image}}" width="800" height="800" alt="{{$cproducts->name}}"></figure>
                                        </a>
                                    </div>
                                    <div class="product-info">
                                        <a href="{{route('product.details',['slug' => $cproducts->slug])}}" class="product-name"><span>{{$cproducts->name}}</span></a>
                                        <div class="wrap-price"><span class="product-price">${{$cproducts->regular_price}}</span></div>
                                    </div>
                                </div>
                                @endforeach
                            </div>
